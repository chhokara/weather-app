...
export default function SearchBar() {
  ...
  return (
    <div className="m-12 flex">
      ...
      <PlacesAutocomplete highlightFirstSuggestion={true} searchOptions={{types: ['(cities)']}} value={location} onChange={setLocation} onSelect={handleSelect}>
        {AutocompleteBlock}
      </PlacesAutocomplete>

      {/* Display Location Button*/}
      <button className="h-8 w-8 p-1 bg-gray-200 rounded-full" onClick={() => {getAddress()}}>
...
const AutocompleteBlock = ({ getInputProps, suggestions, getSuggestionItemProps, loading })=>(
    <div>
      <input className="h-10 bg-transparent outline-none placeholder-primary placeholder-opacity-100 overflow-hidden" {...getInputProps({ placeholder: "Search for places..."})}/>
        <div>
          {suggestions.slice(0,4).map((suggestion, index) => {
            ...
            return <div className="px-4 py-1 rounded-md" key={index} {...getSuggestionItemProps(suggestion, {style})}>{suggestion.description}</div>
          })}
        </div>
    </div>
  );
